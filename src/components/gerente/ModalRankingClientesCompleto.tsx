...
import { TrendingUp, User, ShoppingBag, X, Trophy, Award, Medal } from 'lucide-react'
...
interface ModalRankingClientesCompletoProps {
  isOpen: boolean
  onClose: () => void
  clientes: Cliente[]
...
export function ModalRankingClientesCompleto({
  isOpen,
  onClose,
  clientes,
  currentFilter = 'hoy',
  onFilterChange
}: ModalRankingClientesCompletoProps) {
      ...
      <div className="flex h-[85vh] flex-col">
        ...
        <div className="flex items-center justify-between border-b border-morph-gray-200 pb-4">
          <div>
            ...
            <p className="text-sm text-morph-gray-600 mt-1">
              {filteredClientes.length} clientes ordenados por compras totales
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="mt-4 mb-4 flex gap-3 items-center">
          <input
            ...
            className="flex-1 rounded-lg border border-morph-gray-300 px-4 py-2 focus:border-morph-primary-500 focus:outline-none focus:ring-2 focus:ring-morph-primary-200"
          />
          {onFilterChange && (
            <CompactFilterBar
              value={currentFilter}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-4 border border-yellow-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-yellow-700 font-medium">Top Cliente</p>
            ...
            {clientes[0] && (
              <p className="text-sm text-yellow-700 mt-2">
                ${clientes[0].totalSpent.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
              </p>
            )}
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-bold text-green-900">
                  ${clientes.reduce((sum, c) => sum + c.totalSpent, 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                </p>
              </div>
          ...
          {filteredClientes.length > 0 ? (
            <div className="divide-y divide-morph-gray-200">
              {filteredClientes.map((cliente, index) => (
                <div
                  key={cliente.id}
                  className="flex items-center gap-4 p-4 hover:bg-morph-gray-50 transition-colors"
                >
                  {/* Rank Badge */}
                  ...
                  <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-morph-primary-100">
                    <span className="text-lg font-bold text-morph-primary-700">
